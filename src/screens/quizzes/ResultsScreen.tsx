...
import { Banner, Button, Card, Chip, Icon, Text, useTheme } from 'react-native-paper';
...
export function ResultsScreen({ route, navigation }: Props) {
  ...
  const subject = state.catalog.subjects.find((item) => item.id === (attemptedUnit?.subjectId ?? attemptedPaper?.subjectId));
  ...
  const header = (
    <View style={styles.headerContent}>
      ...
      {attempt.endReason === 'time-expired' ? (
        <Banner visible icon="timer-alert-outline" style={{ backgroundColor: theme.colors.errorContainer }}>
          Time is over. The quiz was submitted automatically and unanswered questions were marked as skipped.
        </Banner>
      ) : null}
      <View style={styles.hero}>
        ...
        <Text variant="bodyMedium" style={styles.muted}>{subject?.name ?? attemptedPaper?.subjectName ?? 'Quiz'} · {attempt.mode === 'exam' ? t('examMode') : t('instantMode')}</Text>
      ...
      <View style={styles.actions}>
        <Button
          mode="contained"
          style={styles.grow}
          onPress={() => attempt.contentType === 'past-paper'
            ? navigation.navigate('PastPaperDetails', { paperId: attempt.unitId })
            : navigation.navigate('QuizDetails', { unitId: attempt.unitId })}
        >
          Try again
        </Button>
