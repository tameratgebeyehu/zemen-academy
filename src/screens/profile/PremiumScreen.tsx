...
const PREMIUM_BENEFITS = [
  ...
  { icon: 'file-document-multiple-outline', title: 'Past papers', body: 'Access and download published exam papers as the archive grows.' },
  { icon: 'download-circle-outline', title: 'Study offline', body: 'Download quizzes and papers, with secure periodic Premium verification.' },
  { icon: 'chart-line', title: 'Complete answer review', body: 'Review correct answers, your choices, and detailed reasoning.' },
...
export function PremiumScreen({}: Props) {
  ...
  const [senderName, setSenderName] = useState(state.user?.isGuest ? '' : state.user?.name ?? '');
  ...
  const showPurchaseFlow = !state.user?.isGuest
    && !state.premiumVerificationRequired
    && !hasOpenRequest
    && !state.user?.isPremium;
  ...
  return (
    <KeyboardAvoidingView style={styles.flex} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
      <Screen>
        ...
        {!state.user?.isPremium && hasOpenRequest && premiumRequest ? (
          <Reveal delay={80}>
            <RequestStatusCard
        ...
        {premiumRequest?.status === 'rejected' && !state.user?.isPremium ? (
          <Card mode="outlined" style={[styles.card, { borderColor: theme.colors.error }]}>
            <Card.Content style={styles.cardContent}>
              <View style={styles.row}>
        ...
        {state.user?.isGuest ? (
          ...
          <>
            ...
            <View style={styles.navigationRow}>
              {step > 1 ? <Button mode="outlined" icon="arrow-left" style={styles.navButton} contentStyle={styles.action} onPress={() => setStep((step - 1) as FlowStep)}>Back</Button> : <View style={styles.navButton} />}
              {step < 3 ? <Button mode="contained" icon="arrow-right" style={styles.navButton} contentStyle={styles.action} disabled={step === 1 ? !selectedPlan : !selectedMethod} onPress={() => setStep((step + 1) as FlowStep)}>Continue</Button> : null}
            </View>
          </>
        ) : null}
        ...
        {!state.user?.isPremium ? <Card mode="contained" style={[styles.noticeCard, { backgroundColor: theme.colors.surfaceVariant }]}>
...
function PremiumHero({ active }: { active: boolean }) {
  return (
    <View style={styles.hero}>
      ...
      <Text variant="bodyLarge" style={styles.heroBody}>{active ? 'Every published subject, quiz, explanation, download, and past paper is open.' : 'One plan unlocks the complete Zemen Academy learning experience.'}</Text>
      <View style={styles.heroTrust}><Icon source="shield-check" size={16} color={GOLD} /><Text variant="labelMedium" style={styles.heroTrustText}>{active ? 'ACTIVE SUBSCRIPTION • SECURE ACCESS' : 'MANUAL PAYMENT • SECURE ACCESS'}</Text></View>
    </View>
  );
}
...
function PremiumActiveCard({ planName, startedAt, until }: { planName: string; startedAt?: string | null; until?: string | null }) {
  ...
  const daysRemaining = Number.isFinite(untilTime) ? Math.max(0, Math.ceil((untilTime - Date.now()) / 86_400_000)) : null;
  return (
...
function SubmitStep({ plan, method, senderName, confirmed, busy, onSenderName, onConfirm, onSubmit }: { plan: PremiumPlan; method: PremiumPaymentMethod; senderName: string; confirmed: boolean; busy: boolean; onSenderName: (value: string) => void; onConfirm: () => void; onSubmit: () => void }) {
  const theme = useTheme();
  return <View style={styles.stepSection}><View style={styles.stepHeading}><Text variant="headlineSmall" style={styles.stepTitle}>Submit for verification</Text><Text variant="bodyMedium" style={styles.muted}>One final detail: enter the exact name shown on the sending bank account.</Text></View><Card mode="outlined" style={styles.card}><Card.Content style={styles.cardContent}><View style={[styles.summaryStrip, { backgroundColor: theme.dark ? '#3A3018' : '#FFF1BD' }]}><Icon source="receipt-text-check-outline" size={25} color={GOLD_BORDER} /><View style={styles.grow}><Text variant="titleMedium" style={styles.bold}>{plan.name} • {plan.priceEtb.toLocaleString()} ETB</Text><Text variant="bodySmall" style={styles.muted}>{method.name} • Date added automatically</Text></View></View><TextInput mode="outlined" label="Name used for the bank transfer" value={senderName} onChangeText={onSenderName} autoCapitalize="words" left={<TextInput.Icon icon="account-outline" />} /><Text variant="bodySmall" style={styles.muted}>Use the account-holder name exactly as it appears in the bank.</Text><View style={styles.autoDetails}><AutoDetail icon="calendar-check" text="Payment date recorded automatically" /><AutoDetail icon="phone-check-outline" text="Account phone included when available" /><AutoDetail icon="identifier" text="A unique request code will be created" /></View><PressableScale onPress={onConfirm}><View style={styles.confirmRow}><Checkbox status={confirmed ? 'checked' : 'unchecked'} /><Text variant="bodySmall" style={styles.confirmText}>I transferred the exact amount to the selected bank and confirm that this sender name is correct.</Text></View></PressableScale><NetworkActivity visible={busy} label="Submitting payment request…" detail="Your request will be ready for manual verification." /><Button mode="contained" icon="shield-check-outline" loading={busy} disabled={busy || !confirmed || senderName.trim().length < 2} contentStyle={styles.action} onPress={onSubmit}>Submit for verification</Button></Card.Content></Card></View>;
}
...
const styles = StyleSheet.create({
  ...
  progress: { flexDirection: 'row', alignItems: 'flex-start', justifyContent: 'center', paddingHorizontal: 10, marginTop: 4 }, progressItem: { width: '33.33%', alignItems: 'center', gap: 5 }, progressDot: { width: 34, height: 34, borderRadius: 17, backgroundColor: '#E6E4DF', alignItems: 'center', justifyContent: 'center' }, progressDotActive: { backgroundColor: GOLD }, progressNumber: { color: '#777' }, progressNumberActive: { color: GOLD_DARK, fontWeight: '900' }, progressLabel: { opacity: 0.66 }, progressLine: { position: 'absolute', left: '66%', top: 16, width: '68%', height: 2, backgroundColor: '#E6E4DF' }, progressLineActive: { backgroundColor: GOLD },
  ...
  benefitsCard: { borderRadius: ui.radius.lg, borderColor: '#D7BF78' }, benefitList: { gap: 14 }, compactBenefits: { gap: 11 }, benefitRow: { flexDirection: 'row', gap: 10, alignItems: 'flex-start' }, benefitIcon: { width: 36, height: 36, borderRadius: 12, backgroundColor: '#FFF1BD', alignItems: 'center', justifyContent: 'center' },
